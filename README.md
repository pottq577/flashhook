...
- **오버엔지니어링 없는 목표 성능 달성**: 비동기 EventPublisher 구조를 통해 단일 인스턴스 톰캣 서버만으로 최대 600 TPS의 웹훅 수신을 큐 적체 현상 없이 안정적으로 소화함을 실측했습니다.
- **대규모 실시간 연결 유지**: 500개의 SSE 커넥션이 연결된 상태에서 초당 400회의 이벤트가 유입되는 고부하 상황에서도, 단 1건의 커넥션 드랍이나 동시성 예외 없이 안정적으로 스트림을 유지함을 검증했습니다.
- **악의적 트래픽 억제**: 웹훅 수신, 엔드포인트 생성 및 Replay 전반에 Rate Limit 방어선을 구축하여, 분당 1200개의 대규모 리플레이 공격 인입 시 한도 초과분을 즉각 방어(`429 Too Many Requests`)하여 시스템 오버헤드를 원천 차단했습니다.
