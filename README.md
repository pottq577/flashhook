...
#### 1. 무분별한 요청 폭주(DDoS) 방어 및 IP Spoofing 차단
- **문제**: 가입 없이 누구나 URL을 생성할 수 있어 매크로 공격에 노출되기 쉽고, `X-Forwarded-For` 헤더를 조작하여 IP 차단을 우회할 위험이 존재했습니다.
- **해결**: Redis의 **고정 윈도우(Fixed Window) 카운터 알고리즘**을 도입하여 즉각적인 Rate Limit(IP당 생성 제한, 엔드포인트당 수신 제한)을 적용했습니다. 또한, 보안 필터(`RateLimitFilter`)에서 조작 가능한 프록시 헤더를 배제하고 물리적 TCP IP(`getRemoteAddr()`)만을 신뢰하도록 방어 로직을 격상했습니다.
