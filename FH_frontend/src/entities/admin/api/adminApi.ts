...
export const adminApi = {
  ...
  getSuspiciousEndpoints: async (): Promise<SuspiciousEndpoint[]> => {
    const res = await fetch(`${API_BASE_URL}/admin/endpoints/suspicious`, {
      headers: getHeaders(),
    });
  ...
  deleteEndpoint: async (endpointId: string): Promise<void> => {
    const res = await fetch(
      `${API_BASE_URL}/admin/endpoints/${encodeURIComponent(endpointId)}`,
      {
        method: "DELETE",
        headers: getHeaders(),
      },
  ...
  removeBlacklistedIp: async (ip: string): Promise<void> => {
    const res = await fetch(
      `${API_BASE_URL}/admin/blacklist/${encodeURIComponent(ip)}`,
      {
        method: "DELETE",
        headers: getHeaders(),
      },
