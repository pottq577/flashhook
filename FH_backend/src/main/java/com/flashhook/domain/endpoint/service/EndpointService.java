...
public class EndpointService {
    ...
    public EndpointResponse create(EndpointCreateRequest request, String ip) {
        ...
        String accessToken =
            com.flashhook.global.security.AccessTokenUtil.generateToken();
        String accessTokenHash =
            com.flashhook.global.security.AccessTokenUtil.hashToken(
                accessToken
            );
        ...
        log.info(
            "Endpoint created successfully: endpointId={}, creatorIp={}",
            endpointId,
            ip
        );
    ...
    public EndpointResponse getInfo(String endpointId) {
        Endpoint endpoint = endpointRepository
            ...
            .orElseThrow(() ->
                new EndpointException(ErrorCode.ENDPOINT_NOT_FOUND)
            );
    ...
    @CacheEvict(value = "endpoints", key = "#endpointId")
    @Transactional
    public EndpointResponse updateMockConfig(
        String endpointId,
        MockUpdateRequest request
    ) {
        Endpoint endpoint = endpointRepository
            .findByEndpointId(endpointId)
            .orElseThrow(() ->
        ...
        if (request.statusCode() != null) mockBuilder.statusCode(
            request.statusCode()
        );
        if (request.delayMs() != null) mockBuilder.delayMs(request.delayMs());
        if (request.headers() != null) mockBuilder.headers(request.headers());
        if (request.body() != null) mockBuilder.body(request.body());
        ...
        if (request.presetOptions() != null) {
            ...
            if (options.containsKey("secretKey")) {
                ...
                if (
                    secretObj instanceof String plainSecret &&
                    !plainSecret.isBlank()
                ) {
                    options.put(
                        "secretKey",
                        encryptionUtil.encrypt(plainSecret)
                    );
                } else {
        ...
        Endpoint updatedEndpoint = endpoint
            .toBuilder()
