...
public interface WebhookLogRepository
    extends MongoRepository<WebhookLog, String>
{
    Page<WebhookLog> findByEndpointId(String endpointId, Pageable pageable);

    @Query(
        "{ 'endpointId': ?0, '$or': [ { 'receivedAt': { '$lt': ?1 } }, { 'receivedAt': ?1, 'logId': { '$lt': ?2 } } ] }"
    )
    Page<WebhookLog> findPreviousPage(
        String endpointId,
        Instant receivedAt,
        String logId,
        Pageable pageable
    ...
    @Query(
        "{ 'endpointId': ?0, '$or': [ { 'receivedAt': { '$gt': ?1 } }, { 'receivedAt': ?1, 'logId': { '$gt': ?2 } } ] }"
    )
    Page<WebhookLog> findNextPage(
        String endpointId,
        Instant receivedAt,
        String logId,
        Pageable pageable
    ...
    Optional<WebhookLog> findFirstByEndpointIdOrderByReceivedAtAsc(
        String endpointId
    );
