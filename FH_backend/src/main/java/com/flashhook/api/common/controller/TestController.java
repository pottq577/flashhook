...
@Profile({ "local", "test" })
@ConditionalOnProperty(name = "flashhook.test-endpoints.enabled", havingValue = "true")
public class TestController {
    ...
    public TestController(StringRedisTemplate redisTemplate, MongoTemplate mongoTemplate, RateLimitService rateLimitService) {
    ...
    private boolean isNotAllowedOrInvalidKey(HttpServletRequest request, String adminKey) {
        ...
        return adminKey == null || !MessageDigest.isEqual(
    ...
    public ResponseEntity<String> cleanupRedis(
        ...
        if (isNotAllowedOrInvalidKey(request, adminKey)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Invalid Admin Key or Rate limit exceeded");
        }
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Redis cleanup failed: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> cleanupMongo(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("MongoDB cleanup failed: " + e.getMessage());
        }
    }
}
