...
개발자들에게 실질적인 테스트 가치를 제공하기 위해 공식 문서를 기반으로 구성한 6개 주요 서비스 프리셋입니다.

#### ① 카카오 (Kakao)

- **REST API**: OAuth 토큰 발급/조회 (`invalid_client`, `invalid_grant` 등 예외 시나리오)
- **Webhook**: 연결 해제, 채널 차단. 카카오 웹훅 서버는 **3초 이내 200 OK**가 필수이므로 '응답 지연' 프리셋으로 타임아웃 테스트.
- [공식 문서 링크](https://developers.kakao.com)

#### ② 토스페이먼츠 (Toss Payments)

- **REST API**: 결제 승인/취소 (`ALREADY_PROCESSED_PAYMENT` 등 에러 코드 재현)
- **Webhook**: 상태 알림. 가맹점 서버가 2xx 응답을 안 하면 **최대 7회 재전송**. 500 에러를 반환하여 중복 방어 로직 검증.
- [공식 문서 링크](https://docs.tosspayments.com)

#### ③ 포트원 V2 (PortOne)

- **REST API**: 결제 단건 조회. `Idempotency-Key` 헤더 누락/중복 검증 테스트.
- **Webhook**: 결제 승인/취소. **웹훅 시그니처 위변조 검증**이 필수. 실패 시 총 5회 재전송.
- [공식 문서 링크](https://developers.portone.io)

#### ④ 솔라피 (Solapi)

- **REST API & Webhook**: 문자 발송 성공/잔액 부족(1030). 수신 서버는 **5초 이내 200 반환** 필수.
- [공식 문서 링크](https://docs.solapi.com)

#### ⑤ 깃허브 (GitHub)

- **Webhook**: Push, PR. 수신 서버는 **10초 이내 응답**해야 하며 **재전송(Retry)을 미지원**하여 드롭됨. `X-Hub-Signature-256` 시그니처 테스트.
- [공식 문서 링크](https://docs.github.com/en/webhooks)

#### ⑥ 슬랙 (Slack)

- **Webhook / Events API**: URL Verification의 `challenge` 에코 반환 요구.
- **재전송**: **3초 이내 미응답 시 최대 3회 재전송**. `X-Slack-No-Retry: 1` 응답 헤더 테스트 가능.
- [공식 문서 링크](https://api.slack.com/events-api)
