...
- **요구사항**: 
  ...
  - 계산된 값을 `webhook-signature` 헤더에 삽입하여 발송
...
- **요구사항**:
  ...
  - `sha256={hash}` 형태로 계산하여 헤더에 주입 후 발송
...
### 인터페이스 설계 (예시)
```java
public interface DynamicPresetHandler {
    /**
     * 해당 핸들러가 처리할 프리셋 타입을 반환합니다.
     */
    String getPresetType();

    /**
     * [수신 파이프라인] Mock 응답을 동적으로 변환하여 반환할 때 사용합니다. (예: Slack Challenge 응답)
     */
    DeferredResult<ResponseEntity<?>> handleResponse(String rawBody, MockConfig mockConfig);

    /**
     * [발송 파이프라인] 웹훅 Replay 또는 Generator 발송 시점에 헤더나 바디를 동적으로 변환할 때 사용합니다. (예: GitHub, PortOne 시그니처)
     */
...
### 컴포넌트 모듈 구성
1. **`SlackPresetHandler`**: `handleResponse`를 구현하여 기존 URL Verification 챌린지 에코(Echo) 기능을 담당합니다.
2. **`PortOnePresetHandler`**: `handleRequestGeneration`을 구현하여 Replay API 파이프라인 실행 시 타임스탬프 갱신 및 HMAC 서명을 수행합니다.
3. **`GitHubPresetHandler`**: `handleRequestGeneration`을 구현하여 Body 해싱 및 `X-Hub-Signature` 헤더 주입을 수행합니다.
