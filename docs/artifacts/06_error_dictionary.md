...
| **400**     | `PRESET_INVALID_CONFIG`   | 프리셋 설정이 올바르지 않습니다            | 모의 응답 프리셋 설정이 잘못되었거나 누락된 경우.                                                             |
| **403**     | `INVALID_TOKEN`           | 유효하지 않은 액세스 토큰입니다            | `fh_token_{endpointId}` 쿠키가 없거나, 변조되었거나, 만료된 경우. (사용자를 홈 화면으로 리다이렉트)          |
| **403**     | `FORBIDDEN`               | 해당 리소스에 접근할 권한이 없습니다       | 본인이 생성하지 않은 엔드포인트에 접근하려 하거나, 블랙리스트 IP 등재, 혹은 SSRF 사설 IP Replay 시도 시 발생. |
...
| **404**     | `NOT_FOUND`               | 요청한 리소스를 찾을 수 없습니다           | 존재하지 않는 API 경로를 요청했을 때.                                                                         |
...
| **429**     | `ENDPOINT_LIMIT_EXCEEDED` | 엔드포인트 생성 제한을 초과했습니다        | 동일 IP에서 10분 내 생성 가능한 엔드포인트 갯수(5개)를 초과했을 때. (제한 만료 후 재시도 안내)                |
| **429**     | `RATE_LIMIT_EXCEEDED`     | 요청 제한을 초과했습니다                   | 1분당 웹훅 수신 100건 초과 또는 1분당 Replay API 20건 초과 시 발생 (DDoS/Spam 방지).                          |
| **500**     | `INTERNAL_ERROR`          | 서버 내부 오류가 발생했습니다              | DB 연결 실패, Redis 타임아웃, 예기치 않은 서버 런타임 익셉션. (잠시 후 다시 시도 안내)                        |
| **500**     | `PRESET_SIGNATURE_FAILED` | 프리셋 서명 생성 중 오류가 발생했습니다    | 외부 발송용 프리셋 서명(HMAC 등) 생성 실패 시 발생.                                                           |
