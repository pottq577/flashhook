...
| --------------- | ----------- | ------ | --------------------------- | :--: |
| 엔드포인트 생성 | 5개/IP      | 10분   | `rl:create2:{ip}`           | 완료 |
| 웹훅 수신       | 100건/EP/IP | 1분    | `rl:hook:{endpointId}:{ip}` | 완료 |
| 웹훅 재전송(Replay)| 20건/EP    | 1분    | `rl:replay:{endpointId}`    | 완료 |
| IP 블랙리스트   | 영구 차단   | 무기한 | `blacklist:ip:{ip}`         | 완료 |

> **엔드포인트 생성 제한**: 무분별한 생성을 막고 개발 편의성을 보장하기 위해 10분 동안 IP당 5개로 제한합니다.
> **웹훅 수신 제한**: 악의적인 도배 요청을 막기 위해 동일 IP에서 특정 엔드포인트로의 요청을 분당 100건으로 제한합니다.
> **웹훅 재전송 제한**: 악의적인 Replay 요청을 방어하기 위해 특정 엔드포인트당 분당 20건으로 제한합니다.
> **IP 블랙리스트**: 악성 트래픽이 감지된 IP는 `blacklist:ip:...` 키를 통해 서비스 접근이 영구 차단(403)됩니다.
...
| 봇 대량 생성                | Rate Limit (5개/IP/10분) 및 블랙리스트       |
...
| Replay 시 SSRF 공격         | 발송 전 사설 IP, 루프백, 링크 로컬 차단 및 IP 핀닝 |
