...
| #   | 항목                                                                        | 유형      | 심각도 | 조치                   |
| --- | --------------------------------------------------------------------------- | --------- | :----: | ---------------------- |
| A1  | 인증: 헤더/`stream-token` → **HttpOnly 쿠키**                               | 문서 낡음 |  높음  | **조치 완료** (`05` 갱신) |
| A2  | Kakao 동적 `presetType` 백엔드 핸들러 부재                                  | 기능 갭   |  중간  | **조치 완료** (정적 응답으로 확정 및 명시) |
| A3  | GitHub/PortOne 요청 서명 = ADR-0002 "Phase 2 제외"인데 이미 구현            | 문서 낡음 |  중간  | **조치 완료** (`adr/0002` 보강) |
| A4  | Redis Pub/Sub 스케일아웃 = 스텁(단일 인스턴스)                              | 기능 미완 |  중간  | **조치 완료** (`04` 문서 명시) |
| A5  | Admin 도메인(`/api/admin/**`) 문서 부재                                     | 문서 누락 |  중간  | **조치 완료** (`05` 보완) |
| A6  | 공개 로그 공유(`/api/public/logs/{id}`, `/session/{id}`) 문서 부재          | 문서 누락 |  중간  | **조치 완료** (`05` 보완) |
| A7  | 에러 코드 표 불일치(`REQUEST_TIMEOUT`/`408` 없음, `PRESET_*` 누락)          | 문서 낡음 |  낮음  | **조치 완료** (`05`/`06` 정정) |
| A8  | `SecurityHeadersFilter` 가 존재하지 않는 경로(`/api/auth`,`/api/user`) 대상 | 잔재 코드 |  낮음  | **조치 완료** (경로 교정 완료) |
| A9  | Base URL/포트/도메인 표기 산발적 불일치                                     | 문서 낡음 |  낮음  | **조치 완료** (`.site` 및 포트 통일) |
