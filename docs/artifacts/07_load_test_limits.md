...
1. **최대 처리량 달성**: 단일 인스턴스로 최대 600 TPS 이상의 웹훅 수신을 큐 적체 현상 없이 안정적으로 소화했습니다.
2. **실시간 팬아웃 방어**: 500개의 SSE 커넥션을 유지한 상태에서 400 TPS의 이벤트를 흩뿌려도 커넥션 드랍이나 동시성 예외가 단 1건도 발생하지 않았습니다.
3. **방어 로직 실증**: Rate Limit 필터가 매우 효율적으로 작동하여 악의적인 대량 리플레이 요청 및 500건 로그 상한을 유도하는 공격을 성공적으로 차단했습니다.

결과적으로 현재 FlashHook의 아키텍처는 별도의 복잡한 스레드 풀 튜닝 없이도 목표치(500 TPS, 500 커넥션)를 여유롭게 방어하는 구조적 안정성을 지니고 있음이 실증되었습니다.
