...
1. **단일 인스턴스 최대 처리량**: 8분간 총 133,853건의 웹훅을 수신하며 에러율 0%를 기록했습니다. 점진적 부하 증가(Ramping) 시나리오 테스트 결과, 테스트 중 평균 8.29ms의 지연 시간(Duration)으로 당초 목표치인 500 TPS를 넘어 **최고 피크 600 TPS**에 달하는 대규모 웹훅 인입을 스레드 큐 적체나 병목 없이 완벽하게 소화함을 실증했습니다.
2. **실시간 팬아웃 방어 (SSE 유실률 0%)**: 최대 500개의 커넥션을 동시에 유지(S3)하며 0% 에러율을 보였고, 팬아웃 환경(S2)에서 웹훅 송신(webhook_success_count) 27,749건 대비 트래픽 종료 후 Grace Period 동안 도달한 이벤트 수(sseEventsReceived)가 정확히 27,749건을 기록하여 유실률 0%를 실증했습니다. 동시 접속자 500명 규모에서 스레드 풀 고갈이나 이벤트 유실(TaskRejectedException)은 발생하지 않았습니다.
3. **방어 로직 실증 (Rate Limit & Log Cap)**:
   - **Rate Limit (S7)**: 웹훅 수신 제한(100/분) 테스트 시 110건의 웹훅 발송 중 최초 100건 허용 후 초과분 10건이 정확히 차단(`429 Too Many Requests`)되었습니다. 엔드포인트 생성 제한(5/10분) 또한 6회 시도 시 5회 성공 후 1회 차단됨을 확인했습니다.
   - **Log Cap (S8)**: 로그 상한(500건) 방어 시나리오를 위해 560건의 웹훅을 발송했으나, load 프로파일 미적용 시 1분당 100건 수신 제한에 의해 최초 100건 허용 후 나머지 460건이 즉각 차단되어 상한 도달 이전에 Rate Limit가 먼저 시스템을 방어함을 관찰했습니다.

결과적으로 현재 FlashHook의 아키텍처는 별도의 복잡한 튜닝 없이도 당초 목표치(500 커넥션, 500 TPS)를 넘어 최고 600 TPS 수준의 부하를 단일 인스턴스로 여유롭게 방어하는 구조적 안정성을 지니고 있음이 실증되었습니다.
