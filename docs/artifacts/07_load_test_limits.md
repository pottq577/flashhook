...
> **💡 Note**: 아래 내용은 **부하 테스트 전의 아키텍처 기반 예측**입니다. 실제 k6 부하 테스트를 진행한 결과, 예측(400 TPS부터 대량 유실)과 달리 **실제 유실률은 0.031%로 극히 미미**하여 시스템이 예상보다 훨씬 견고한 것으로 나타났습니다. 튜닝의 필요성은 예측보다 낮아졌으며, 자세한 실측 결과는 **§6. 실측 부하 테스트 결과**를 참조하세요.
...
| **S1**   | Webhook Capacity    | 8분 (480s)     | **평균 278.7 TPS** (133,799/480s)                 | p95: `12.1ms`, Avg: `6.6ms`                    | Error 0%. (피크 구간 목표 600 TPS 무오류 도달 과정 포함)               |
| **S2**   | SSE Fanout (1:5)    | 약 2분 40초    | **Webhook**: 27,749건<br>**SSE Event**: 138,701건 | Webhook p95: `16.6ms`                          | 하단 [S2 분석 상세] 참조                                               |
| **S3**   | SSE Scale           | 1분            | 500 VUs 연결 유지                                 | p95: `5.7ms`, Avg: `3.1ms`                     | 이번 실측은 1분 스모크 목적. 30분 장기 Soak 안정성은 별도 예정         |
...
이 S2 시나리오의 최대 부하는 §2.B에서 예측했던 임계점(약 400 TPS)과 정확히 맞닿아 있습니다. **실제 해당 피크 구간에서 소량의 거부(19건)가 관측되어 예측 모델의 방향성은 유효했으나, 대규모 유실이 발생할 것이라는 예상과 달리 유실률 자체는 목표치인 0.1% 미만(0.031%)에 그쳐 절대적 심각도는 예측보다 훨씬 낮았습니다.**
현재 유실 원인 중 `taskExecutor` 큐 포화가 가장 유력하나 팬아웃/Heartbeat의 정확한 기여도는 아직 미분리 상태입니다. 추후 `Counter.builder("executor.rejected.tasks").tag("task_type", "webhook_fanout"|"heartbeat")`와 같이 태그를 세분화하여 재측정하면 진짜 병목 원인을 구체화할 수 있습니다.
...
부하 테스트 수행 중 Actuator/Prometheus를 통해 백엔드 서버의 상태를 동시 관찰한 결과는 다음과 같습니다.

- **JVM 및 시스템 자원**: JVM 힙 사용률은 피크 시에도 약 300MB(G1 GC) 수준으로 힙 포화나 빈번한 Full GC가 발생하지 않았으며, Tomcat busy 스레드와 Mongo 커넥션 풀 또한 포화되지 않고 안정적인 상태를 유지했습니다. (특이사항 없음)
- **Redis 지표 (측정 과제)**: §2.A에서 지적된 대로 Lettuce 클라이언트의 단일 연결 특성상 고부하 시 Redis 지연이 Rate Limit 성능에 영향을 줄 수 있으나, 현재 Spring Boot 설정상 `redis_command_duration` 같은 상세 지표가 노출되지 않아 실측이 불가능했습니다. 향후 정밀한 병목 튜닝을 위해 Redis 상세 지표 노출은 후속 백로그 작업으로 남겨둡니다.
