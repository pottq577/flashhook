...
초기 예측(400 TPS 한계)과 달리, 실제 k6 부하 테스트를 통해 모든 시나리오(S0~S8)를 검증한 결과 다음과 같은 성능 지표가 확인되었습니다.

1. **단일 인스턴스 최대 처리량**: 8분간 총 133,853건의 웹훅을 수신하며 에러율 0%를 기록했습니다. 테스트 중 평균 8.29ms의 지연 시간(Duration)으로 최대 300 TPS (Virtual Users: 50) 이상의 웹훅 인입을 병목 없이 소화했습니다.
2. **실시간 팬아웃 방어 (SSE)**: 최대 500개의 커넥션을 동시에 유지(S3)하며 0% 에러율을 보였고, 팬아웃 환경(S2)에서도 27,853건의 요청을 평균 7.70ms 이내에 처리했습니다. 동시 접속자 500명 규모에서 스레드 풀 고갈이나 이벤트 유실(TaskRejectedException)은 발생하지 않았습니다.
3. **방어 로직 실증 (Rate Limit & Log Cap)**:
   - 분당 1200개의 대규모 리플레이 공격 인입 시 120건 중 100건 허용 후 즉시 10건 차단(`429 Too Many Requests`)이 발생하여 한도 초과분을 방어했습니다. (S7)
   - 한 엔드포인트에 566건 이상의 웹훅을 발송했을 때 최초 100건만 허용되고 나머지 460건이 즉각 차단되며 어뷰징을 원천 차단함을 확인했습니다. (S8)

결과적으로 현재 FlashHook의 아키텍처는 별도의 복잡한 튜닝 없이도 목표치(500 커넥션, 300~500 TPS) 수준을 단일 인스턴스로 여유롭게 방어하는 구조적 안정성을 지니고 있음이 실증되었습니다.
