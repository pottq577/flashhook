...
1. **단일 인스턴스 최대 처리량**: 8분간 총 133,853건의 웹훅을 수신하며 에러율 0%를 기록했습니다. 테스트 중 평균 8.29ms의 지연 시간(Duration)으로 **최대 300 TPS** (Virtual Users: 50) 수준의 웹훅 인입을 병목 없이 소화함을 확인했습니다. (목표치인 500 TPS는 미검증 상태이며, 현재 확인된 한계선은 300 TPS입니다.)
...
결과적으로 현재 FlashHook의 아키텍처는 별도의 복잡한 튜닝 없이도 확인된 300 TPS 및 500 커넥션 수준을 단일 인스턴스로 여유롭게 방어하는 구조적 안정성을 지니고 있음이 실증되었습니다.
